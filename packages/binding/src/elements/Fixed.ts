...
export default class Fixed extends Element {
  static propTypes = {
  ...
  constructor(props: object = {}) {
    super(new Gtk.Fixed(), props, { isContainer: true });
  }

  move(widget: object, x: number, y: number): null {
    return this.node.move(widget, x, y);
  }

  put(widget: object, x: number, y: number): null {
    return this.node.put(widget, x, y);
  }
